...
impl Buffers {
    pub fn new(context: Rc<Context>, link: ComponentLink<Editor>) -> Self {
        Self {
            context,
            link,
            buffers: Vec::new(),
...
impl Buffer {
    fn new(
        ...
        if let Some(parser) = parser.as_mut() {
            ...
            parser.ensure_tree(
                ...
                || text.clone(),
                move |status| {
                    link.send(BuffersMessage::new(id, BufferMessage::ParseSyntax(status)).into())
                },
            );
        };

        let content = EditTree::new(text);
        Self {
            context,
            link,
            id,
    ...
    pub fn handle_message(&mut self, message: BufferMessage) {
        match message {
            ...
            BufferMessage::ParseSyntax(parsed) => {
                log::info!("done!");
    ...
    fn handle_cursor_message(&mut self, cursor_id: CursorId, message: CursorMessage) {
        {
            ...
            // Stateless
            match message {
                CursorMessage::Up(n) => cursor.move_up_n(&self.content, n),
                CursorMessage::Down(n) => cursor.move_down_n(&self.content, n),
                CursorMessage::Left => cursor.move_left(&self.content),
                CursorMessage::Right => cursor.move_right(&self.content),
                CursorMessage::StartOfLine => cursor.move_to_start_of_line(&self.content),
                CursorMessage::EndOfLine => cursor.move_to_end_of_line(&self.content),
                CursorMessage::StartOfBuffer => cursor.move_to_start_of_buffer(&self.content),
                CursorMessage::EndOfBuffer => cursor.move_to_end_of_buffer(&self.content),

                CursorMessage::BeginSelection => cursor.begin_selection(),
                CursorMessage::ClearSelection => {
                    cursor.clear_selection();
                }
                CursorMessage::SelectAll => cursor.select_all(&self.content),
    ...
    fn update_parse_tree(&mut self, diff: &OpaqueDiff, fresh: bool) {
        if let Some(parser) = self.parser.as_mut() {
            ...
            parser.spawn(
                ...
                self.content.staged().clone(),
                fresh,
                move |status| {
                    link.send(
                        BuffersMessage::new(buffer_id, BufferMessage::ParseSyntax(status)).into(),
                    )
                },
            );
        }
...
impl BufferCursor {
    ...
    #[inline]
    pub fn move_to_start_of_line(&self) {
        self.send_cursor(CursorMessage::StartOfLine);
    }

    #[inline]
    pub fn move_to_end_of_line(&self) {
        self.send_cursor(CursorMessage::EndOfLine);
    }

    #[inline]
    pub fn move_to_start_of_buffer(&self) {
        self.send_cursor(CursorMessage::StartOfBuffer);
    }

    #[inline]
    pub fn move_to_end_of_buffer(&self) {
